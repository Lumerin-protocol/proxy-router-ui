...
export class EthereumGateway {
  ...
  async purchaseContract(props: { contractAddress: string, validatorAddress: string, encrValidatorURL: string, encrDestURL: string, termsVersion: string, buyer: string, price: string, ethValue: string }): Promise<SendStatus> {
  ...
  async closeContract(props: { contractAddress: string, from: string, fee: string, closeoutType: CloseOutType }): Promise<SendStatus> {
  ...
  async getContract(contractAddress: string) {
    const data = await Implementation(this.web3, contractAddress).methods.getPublicVariables().call();
  ...
  async editContractDestination(props: { contractAddress: string, from: string, encrValidatorURL: string, encrDestURL: string }): Promise<SendStatus> {
    const impl = Implementation(this.web3, props.contractAddress);
    const gas = await impl.methods
  ...
  async editContractTerms(props: { contractAddress: string, from: string, price: string, speed: string, length: string, profitTarget: string, termsVersion: string }): Promise<SendStatus> {
    const impl = Implementation(this.web3, props.contractAddress);
    const gas = await impl.methods
